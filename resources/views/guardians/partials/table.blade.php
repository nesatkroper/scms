        ...
        @if (count($guardians) > 0)
            @foreach ($guardians as $guardian)
                <tr class="text-nowrap border-b border-gray-200 dark:border-gray-700 hover:bg-indigo-50 dark:hover:bg-gray-700">
                    ...
                    <td class="px-4 py-2">{{ Str::limit($guardian->occupation, 20) }}</td>
                    <td class="px-4 py-2">{{ Str::limit($guardian->company, 20) }}</td>
                    <td class="px-4 py-2">{{ $guardian->relation }}</td>
                    {{-- <td class="px-4 py-2">
                        @if($guardian->students->count() > 0)
                            {{ $guardian->students->count() }} student(s)
                        @else
                            <span class="text-gray-400">None</span>
                        @endif
                    </td> --}}
                    <td class="px-4 py-2">
                          {{ \Carbon\Carbon::parse($guardian->created_at)->format('Y-m-d') }}
                    </td>
                        ...
                        <div class="relative">
                            ...
                            <div class="dropdown-menu hidden absolute w-auto right-0 z-10 mt-2 origin-top-right divide-y divide-gray-100 dark:divide-gray-700 rounded-md bg-white dark:bg-gray-800 shadow-lg ring-1 ring-black/5 dark:ring-gray-700 focus:outline-none"
                                ...
                                <div class="py-1" role="none">
                                    <a href="#" title="Edit Id({{ $guardian->id }})"
                                    ...
                                    <a href="#" title="Details Id({{ $guardian->id }})"
                                    ...
                                    <button href="#" title="Delete Id({{ $guardian->id }})"
