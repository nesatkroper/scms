    ...
    <tbody class="bg-white dark:bg-gray-800">
      @forelse ($data as $index => $row)
        <tr class="border-b dark:border-gray-700">
          ...
          <td class="p-2 text-gray-700 dark:text-gray-300 capitalize">
            {{ $row->exam->courseOffering->subject->name ?? __('message.n/a') }}
            ({{ $row->exam->courseOffering->teacher->name ?? __('message.n/a') }} -
            {{ $row->exam->courseOffering->time_slot ?? __('message.n/a') }})
          </td>
