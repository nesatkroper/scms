      ...
      <li class="menu-item relative">
        ...
        <div class="submenu">
          <ul class="pl-2 pr-2">
            ...
            <li class="relative">
              <a href="{{ route('admin.expenses.index') }}" class="flex items-center justify-between px-3 py-2 hover:bg-indigo-700
  ...
  <div class="sidebar-footer relative">
    ...
    <div id="sidebar-footer-dropdown"
      ...
      <ul>
        ...
        <li>
          <form method="POST" action="{{ route('logout') }}" class="block hover:bg-indigo-700 dark:hover:bg-gray-700">
            @csrf
    ...
    <div id="sidebar-footer-collapsed" class="p-4 bg-indigo-900/50 dark:bg-gray-900/50 cursor-pointer hidden ">
      <div class="flex justify-center">
        ...
        @else
          <img src="{{ asset('images/user.png') }}" alt="User" class="size-8 rounded-full border-2 border-indigo-500">
          <span class="footer-tooltip">{{ auth()->user()->name }}<br>{{ auth()->user()->email }}</span>
        @endif
      </div>
    </div>
