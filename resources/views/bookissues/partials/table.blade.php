<table class="w-full text-sm text-left">
    <thead class="bg-gray-50 dark:bg-gray-700/50 text-xs text-gray-700 dark:text-gray-400 uppercase">
        <tr class="text-nowrap">
            ...
            <th scope="col" class="px-4 py-4">Book</th>
            <th scope="col" class="px-4 py-4">Patron</th>
            <th scope="col" class="px-4 py-4">Issue Date</th>
            <th scope="col" class="px-4 py-4">Due Date</th>
            <th scope="col" class="px-4 py-4">Return Date</th>
            <th scope="col" class="px-4 py-4">Status</th>
            <th scope="col" class="px-4 py-4">Fine</th>
    ...
    <tbody>
        @if (count($bookIssues) > 0)
            @foreach ($bookIssues as $issue)
                ...
                {{ $issue->status == 'overdue' ? 'bg-red-50 dark:bg-red-900/20' : '' }}">
                    <!-- ID -->
                        ...
                        <div class="flex items-center gap-2">
                            <div class="flex-shrink-0 h-8 w-8">
                                <img class="h-8 w-8 rounded-sm object-cover"
                                    src="{{ $issue->book->cover_image_url ?? asset('images/default-book.png') }}"
                                    alt="{{ $issue->book->title }}">
                            </div>
                            <div>
                                <div class="font-medium">{{ $issue->book->title }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">ISBN: {{ $issue->book->isbn }}
                                </div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-4 py-2">
                        ...
                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $issue->user->email }}</div>
                    </td>
                    ...
                    <!-- Due Date -->
                    <td
                        class="px-4 py-2 {{ $issue->isOverdue() ? 'text-red-600 dark:text-red-400 font-semibold' : '' }}">
                        {{ $issue->due_date->format('Y-m-d') }}
                    </td>

                    <!-- Return Date -->
                    <td class="px-4 py-2">
                        {{ $issue->return_date ? $issue->return_date->format('Y-m-d') : 'Not returned' }}
                    </td>
                    ...
                    <td class="px-4 py-2">
                        ...
                        {{ $issue->status == 'issued' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' : '' }}
                        {{ $issue->status == 'returned' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : '' }}
                        {{ $issue->status == 'overdue' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' : '' }}">
                        ...
                        <div class="relative">
                            ...
                            <div class="dropdown-menu hidden absolute w-auto right-0 z-10 mt-2 origin-top-right divide-y divide-gray-100 dark:divide-gray-700 rounded-md bg-white dark:bg-gray-800 shadow-lg ring-1 ring-black/5 dark:ring-gray-700 focus:outline-none"
                                ...
                                <div class="py-1" role="none">
                                    ...
                                    <a href="#" title="Details Id({{ $issue->id }})"
                                    ...
                                    <button href="#" title="Delete Id({{ $issue->id }})"
                    ...
                    <!-- Checkbox -->
                    <td class="px-2 py-2">
                        <input type="checkbox" name="selected_ids[]" value="{{ $issue->id }}"
