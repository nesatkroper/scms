<div x-show="showRoleModal" x-cloak x-transition.opacity
  ...
  <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-xl">
    <!-- Header -->
    <div class=" p-4 py-2 text-white rounded-[1px_12px_0px_12px]">
    ...
    <form :action="'/admin/users/role/' + user.id + ''" method="POST" class="p-4">
          ...
          @foreach ($roles as $role)
            <label class="block cursor-pointer group">
              <input type="checkbox" name="role_names[]" value="{{ $role->name }}"
                :checked="selectedRoles.includes('{{ $role->name }}')" x-model="selectedRoles" class="hidden peer">

              <!-- Role Card -->
              <div class="border-1 border-gray-200 dark:border-gray-600 rounded-lg p-2
                ...
                <div class="flex items-center justify-between">
                  <div class="flex items-center gap-2">
                    <!-- Role Icon -->
                    <div class="flex-shrink-0 size-8 rounded-lg
                  ...
                  <!-- Checkbox Indicator -->
                  <div class="flex-shrink-0 w-6 h-6 border-1 border-gray-500  dark:border-gray-500 rounded
                                  flex items-center justify-center transition-all duration-200" :class="{
                              'bg-indigo-600 border-indigo-500 dark:border-indigo-400 dark:bg-indigo-500': selectedRoles
                                  .includes('{{ $role->name }}')
                          }">
                    <i class="ri-checkbox-circle-line text-xl transition-opacity duration-200" :class="{
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
        <button @click.prevent="closeModals()" type="button" class="px-5 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300
        ...
        <button type="submit" :disabled="selectedRoles.length === 0" class="px-5 py-2.5 text-sm font-medium text-white
