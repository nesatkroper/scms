  ...
  <table class="min-w-full border border-gray-300 dark:border-gray-700 rounded-lg text-sm">
    <thead class="bg-gray-100 dark:bg-gray-700">
      <tr>
        ...
        <th class="p-2 text-center text-gray-700 dark:text-gray-200 border-b dark:border-gray-600">
          {{ __('message.grade') }}</th>
      </tr>
    </thead>
      ...
      @forelse ($data as $index => $row)
        <tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900">
          ...
          <td class="p-2 text-center text-gray-700 dark:text-gray-300">
            {{ number_format($row->attendance_grade ?? 0, 0) }}</td>
          <td class="p-2 text-center text-gray-700 dark:text-gray-300">
            {{ number_format($row->listening_grade ?? 0, 0) }}</td>
          <td class="p-2 text-center text-gray-700 dark:text-gray-300">{{ number_format($row->writing_grade ?? 0, 0) }}
          </td>
          <td class="p-2 text-center text-gray-700 dark:text-gray-300">{{ number_format($row->reading_grade ?? 0, 0) }}
          </td>
          <td class="p-2 text-center text-gray-700 dark:text-gray-300">
            {{ number_format($row->speaking_grade ?? 0, 0) }}</td>
          <td class="p-2 text-center text-gray-700 dark:text-gray-300">{{ number_format($row->midterm_grade ?? 0, 0) }}
          </td>
          <td class="p-2 text-center text-gray-700 dark:text-gray-300">{{ number_format($row->final_grade ?? 0, 0) }}
          </td>
