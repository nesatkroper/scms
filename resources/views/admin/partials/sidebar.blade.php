      ...
      @if (Auth::user()->hasPermissionTo('view_course-offering') && !Auth::user()->hasRole('admin'))
        <li class="menu-item relative">
          <a href="{{ route('admin.course_offerings.index') }}"
            class="flex items-center justify-between px-4 py-3 hover:bg-indigo-700 dark:hover:bg-gray-700 rounded-lg mx-2 transition-all duration-200
                                      {{ request()->routeIs('admin.course_offerings*') ? 'bg-indigo-700' : '' }}">
            <div class="wr-icon flex items-center">
              <i class="fas fa-tachometer-alt text-center"></i>
              <span class="ml-3 sidebar-text text-hidden">{{ __('message.course_offering') }}</span>
            </div>
            <span class="menu-tooltip">{{ __('message.course_offering') }}</span>
          </a>
        </li>
      @endif
