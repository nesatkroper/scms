        ...
        <form action="{{ route('guardians.store') }}" method="POST" enctype="multipart/form-data" class="py-4">
            ...
            <div class="h-[65vh] md:h-auto overflow-y-auto px-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-1 sm:gap-4 mb-2">
                    <!-- Guardian Field -->
                    <div class="mb-2">
                        <label for="guardian_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                        ...
                        <select id="guardian_id" name="guardian_id" required
                            ...
                            @error('guardian_id') border-red-500 @else border-gray-400 @enderror">
                            <option value="">Select Guardian</option>
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('guardian_id') == $user->id ? 'selected' : '' }}>
