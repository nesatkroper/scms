@if (count($guardians) > 0)
    @foreach ($guardians as $guardian)
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-lg">
            <!-- Profile Image -->
            @if ($guardian->user && $guardian->user->profile_photo_path)
                <div class="h-48 overflow-hidden">
                    <a title="{{ $guardian->user->name }}" class="detail-btn cursor-grab" data-id="{{ $guardian->id }}">
                        <img src="{{ asset('storage/' . $guardian->user->profile_photo_path) }}" alt="{{ $guardian->user->name }}"
            ...
            <div class="p-4">
                <div class="flex justify-between items-start">
                    <div>
                        <h4 class="font-bold text-lg text-gray-800 dark:text-gray-200">
                            @if($guardian->user)
                                {{ Str::limit($guardian->user->name, 25) }}
                            @else
                ...
                <div class="mt-3 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <span>{{ $guardian->students->count() }} student(s)</span>
                    </div>
                </div>
