@if (count($gradelevels) > 0)
        ...
        <div
            ...
            <div class="p-4">
                <div class="flex justify-between items-start">
                    <div>
                        <h4 class="font-bold text-lg text-gray-800 dark:text-gray-200">{{ $gradelevel->name }}</h4>
                        <span class="text-sm text-indigo-600 dark:text-indigo-400">{{ $gradelevel->code }}</span>
                    </div>
                ...
                <div class="mt-3 text-sm text-gray-600 dark:text-gray-400">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <span>{{ $gradelevel->department?->name ?? 'No Department' }}</span>
                    </div>
                    <div class="flex items-center gap-2 mb-1">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                        ...
                        <span>{{ $gradelevel->credit_hours }} Credit Hours</span>
                    </div>
                </div>

                <p class="mt-3 text-sm text-gray-500 dark:text-gray-400 line-clamp-2">{{ $gradelevel->description }}</p>
    ...
    @endforeach
@else
    <p class="p-4 text-center text-red-500 dark:text-red-600">
