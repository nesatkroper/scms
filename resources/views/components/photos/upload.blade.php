<div class="absolute -bottom-12">
  <div
    ...
    <input type="file" id="{{ $edit ? "upload_$name" : "$name" }}" name="{{ $name }}"
      accept="image/jpeg,image/png,image/webp,image/gif" max="2048" class="hidden">
      ...
      <div class="md:w-64 flex flex-col gap-3">
        ...
        <div class="flex gap-2 mt-auto pt-4">
          <button type="button" id="applyEditCrop"
            class="cursor-pointer p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex-1">
            Apply
          </button>
        </div>
      </div>
...
@push('scripts')
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      // {{ __('message.edit') }} Modal Photo Upload with Cropper.js
