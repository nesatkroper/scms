        ...
        @if (count($students) > 0)
            @foreach ($students as $student)
                <tr class="text-nowrap border-b border-gray-200 dark:border-gray-700 hover:bg-indigo-50 dark:hover:bg-gray-700">
                    <td class="px-4 py-2 whitespace-nowrap text-gray-900 dark:text-white">
                        <div class="flex items-center">
                            <img class="detail-btn size-10 rounded-full object-cover cursor-pointer"
                                src="{{ $student->photo ? asset($student->photo) : 'https://placehold.co/40x40/6366F1/FFFFFF?text=' . substr($student->name, 0, 1) }}"
                                alt="{{ $student->name }} image" data-id="{{ $student->id }}">
                                 {{-- <img class="detail-btn size-10 rounded-full object-cover cursor-pointer"
                                src="{{ $student->photo ? asset('storage/'.$student->photo) : 'https://placehold.co/40x40/6366F1/FFFFFF?text=' . substr($student->name, 0, 1) }}"
                                alt="{{ $student->name }} image" data-id="{{ $student->id }}"> --}}
                            <div class="pl-3">
                    ...
                    <td class="px-4 py-2">{{ $student->phone ?? 'N/A' }}</td>
                        ...
                        <div class="relative">
                            ...
                            <div class="dropdown-menu hidden absolute w-auto right-0 z-10 mt-2 origin-top-right divide-y divide-gray-100 dark:divide-gray-700 rounded-md bg-white dark:bg-gray-800 shadow-lg ring-1 ring-black/5 dark:ring-gray-700 focus:outline-none"
                                ...
                                <div class="py-1" role="none">
                                    <a href="#" title="Edit Id({{ $student->id }})"
                                    ...
                                    <a href="#" title="Details Id({{ $student->id }})"
                                    ...
                                    <button href="#" title="Delete Id({{ $student->id }})"
                ...
                <td colspan="12" class="p-4 text-center">
                    <div class="col-span-full py-12 text-center">
                        <div class="max-w-md mx-auto p-6 bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                            ...
                            <p class="mt-2 text-sm text-red-500 dark:text-red-500">Create your first student to get started</p>
                        </div>
                    </div>
                </td>
            </tr>
