@if (count($guardians) > 0)
    @foreach ($guardians as $guardian)
        <div class="bg-white dark:bg-slate-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-lg">
            <!-- Profile Image -->
            <div class="h-48 flex justify-center items-center">
                @if($guardian->photo)
                    <a title="{{ $guardian->name }}" class="detail-btn cursor-grab" data-id="{{ $guardian->id }}">
                        <img src="{{ asset($guardian->photo) }}" alt="{{ $guardian->name }}" class="w-[160px] h-[160px] m-auto object-cover rounded-full border-4 border-slate-100 dark:border-slate-600">
                    </a>
                @else
            ...
            <div class="p-4">
                <div class="flex justify-between items-start">
                    ...
                    <div class="flex items-center gap-1">
                        <button class="btn detail-btn p-2 flex items-center justify-center rounded-full size-8 cursor-pointer text-indigo-500 hover:bg-indigo-100 dark:hover:bg-gray-900 transition-colors" data-id="{{ $guardian->id }}">
                            <span class="btn-content">
                ...
                <div class="flex justify-end gap-2 mt-4">
                    <button class="btn edit-btn p-2 flex items-center justify-center rounded-full size-8 cursor-pointer text-green-500 hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors" data-id="{{ $guardian->id }}">
                        <span class="btn-content">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <button class="delete-btn p-2 flex items-center justify-center rounded-full size-8 cursor-pointer text-red-500 hover:bg-red-100 dark:hover:bg-gray-900 transition-colors" data-id="{{ $guardian->id }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
