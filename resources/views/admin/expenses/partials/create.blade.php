...
<div id="Modalcreate" class="fixed inset-0 z-50 flex items-center justify-center p-4 hidden">
    <div
        ...
        <!-- Form Content -->
        <form action="{{ route('expenses.store') }}" method="POST" class="p-4 needs-validation" novalidate>
            @csrf

            <div class="h-[65vh] md:h-auto">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-1 sm:gap-4 mb-2">
                    <!-- Name Field -->
                    <x-fields.input label="Title Name" name="title" placeholder="Enter title name"
                        :required="true" />
                    <!-- Amount Field -->
                    <x-fields.input type="number" min="0" label="Amount" name="amount"
